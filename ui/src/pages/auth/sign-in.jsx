...
export function SignIn() {
  ...
  const ShowError = (error) => {
    if (error) {
      dispatch({ type: 'errorMsg', value: error });
      $('.errodmesg').fadeIn();
    }
    setTimeout(() => {
      $('.errodmesg').fadeOut();
      ...
      axios.post('https://65.0.173.137/api/login/', { "username": state.username, "password": state.password })
        .then(function (response) {
          ...
          if (response.data.status == "success") {
            dispatch({ type: 'notification', value: { message: "Signed in Successfully", colour: "green" } });
            localStorage.setItem("token", response.data.data.token);
            //return <Navigate  to='/dashboard/home' replace={true}  />
            setTimeout(() => { navigateTo("/dashboard/home"); }, 1000)
          }
        })
        .catch(function (error) {
          // handle error
          dispatch({ type: 'notification', value: { message: error.response.data.message, colour: "red" } });
        })
        .finally(function () {
          // always executed
          setloader(false)
...
{/* 
      {state.loader ? <div className="absolute" style={{ top: 0, left: 0, width: "100%", height: "100%", background: "#00000021", }}>
      ...
      <div className="w-full lg:w-3/5 mt-24">
        ...
        <form className="mt-8 mb-2 mx-auto w-80 max-w-screen-lg lg:w-1/2">
          <div className="mb-1 flex flex-col gap-6">
            ...
            />
            {state.errorMsg && <p className="errodmesg text-red-700">{state.errorMsg.username}</p>}
            <Typography variant="small" color="blue-gray" className="-mb-3 font-medium">
              Password
            </Typography>
            <Input
            ...
            />
            {state.errorMsg && <p className="errodmesg text-red-700">{state.errorMsg.password}</p>}
          </div>
          <Checkbox
            label={
              <Typography
          ...
          />
          <Button className="mt-6" fullWidth onClick={(e) => SignInButton()}>
            Sign In
          </Button>
